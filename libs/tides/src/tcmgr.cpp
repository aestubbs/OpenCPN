...
std::wstring TCMgr::GetTidalEventStr(int station_id, QDateTime ref_dt,
  ...
  if (event > 0) {
    event_str.append(L": ");
    // NOTE: the wx build ran event_dt through toUsrDateTime() for the user's
    // UTC/local/LMT preference (dt_type/lon); the de-wx'd engine formats the
    // raw event time for now -- timezone-mode selection is a follow-up.
    event_str.append(
        event_dt.toString("yyyy-MM-dd HH:mm:ss").toStdWString());
  }
